import { FormService, FormServiceOptions, FormState } from '@/infrastructure/form/FormService';
...
export const useForm = <T extends Record<string, unknown>>(options: Options<T>) => {
  const [innerState, setInnerState] = useState<FormState<T>>(new FormState<T>(options?.initialData || ({} as T)));
